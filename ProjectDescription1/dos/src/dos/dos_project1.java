/*
    ...
    Date: 06-01-2024
    OS: Windows 11 (Powershell Terminal 2.0)
*/
...
public class dos_project1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            System.out.println();
            System.out.println("Chooose the scheduling algorithm:");
            System.out.println("1. First-Come, First-Served (FCFS)");
            System.out.println("2. Round Robing(RR)");
    ...
    // method for calculate and display the time
    private static void calculateAndDisplayTimes(int n, int[] burstTimes, int[] arrivalTimes) {
        int[] wt = new int[n];
        int[] rt = new int[n];
        int[] ct = new int[n];
        wt[0] = 0; 
        ...
        float att = 0; 
        for (int i = 0; i < n; i++) { 
            System.out.println((i + 1) + "\t ||\t" + burstTimes[i] + "\t||\t" + arrivalTimes[i] + "\t||\t" + wt[i] + "\t||\t "+ rt[i] + "\t||\t " + ct[i]); 
            awt += wt[i]; 
            art += rt[i]; 
    ...
    // method for finding the average time
    protected static void findAvgTime(int[] processes, int n, int[] burstTimes, int quantum, int[] arrivalTimes) {
    ...
    static void findWaitingTime(int[] processes, int n, int[] burstTimes, int[] wt, int quantum, int[] arrivalTimes, int[] ct, int[] rt) {
    ...
    static void findTurnAroundTime(int[] processes, int n, int[] burstTimes, int[] wt, int[] tat, int[] ct, int[] arrivalTimes) {
        for (int i=0; i<n; i++) {
            tat[i] = ct[i] - arrivalTimes[i];
        }
    }

    // method to calculate FCFS average waiting time
    static double calculateFCFSAvgWaitingTime(int[] burstTimes, int[] arrivalTimes) {
    ...
    // compareAlgorithmsEfficiency
    static void compareAlgorithmsEfficiency(double avgWaitingTimeRR, double avgWaitingTimeFCFS) {
